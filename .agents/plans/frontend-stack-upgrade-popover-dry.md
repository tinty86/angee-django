...
>   Extend with **feature parity vs Odoo 18 groups + UX** — research in `.agents/notes/grouped-list-odoo-parity.md` (dedicated agent, in progress: studies `_read_group`/`web_read_group`, the web relational group model, and the list renderer's folded headers + two-level pagers + group aggregates). Spans **backend GraphQL** (strawberry-django-aggregates must serve per-group counts/aggregates AND per-group paginated items), **SDK** (group-pagination + per-group item-pagination + aggregate hooks/state), and **ListView/DataToolbar** (folded group rows, group-level pager, per-group item pager, aggregate footer). Fold into this slice once the research note lands.
...
- [ ] done [ ] verified [ ] checked by human (final qa) — FormView full chrome (smart-buttons, More menu, nav guard, inline title input) + stories (gate against the mockup fidelity fingerprint)
